...
USE master;
GO
IF EXISTS (SELECT * FROM sysdatabases WHERE name='ParkFacilAdmDB')
BEGIN
	ALTER DATABASE ParkFacilAdmDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE
END
GO
USE master;
IF EXISTS(SELECT * FROM sysdatabases where name='ParkFacilAdmDB')
BEGIN
	DROP DATABASE ParkFacilAdmDB	
END
GO
CREATE DATABASE ParkFacilAdmDB;
GO
USE [ParkFacilAdmDB];
GO
